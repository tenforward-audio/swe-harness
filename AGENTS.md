...
- Stack: Markdown and YAML, with a Python standard-library validator.
- Architecture: this root contract routes intent; `.agents/` owns detailed
  policy, live work state, capability declarations, and repo-local skills;
  `scripts/` enforces structural invariants.
...
- Type or static checks: `python3 -m py_compile scripts/check_harness.py`
- Tests: `python3 -m unittest discover -s tests`
- Build: not applicable; this project distributes repository files directly.
- Cross-stack checks without builds: `python3 scripts/check_harness.py`
- Full checkpoint and release gate:
  `python3 -m unittest discover -s tests && python3 scripts/check_harness.py`
